"""
model_official_v35.py —— 官方 Polynormer 模型（保底分支 / 复现基线）

Polynormer (ICLR 2024) 的本地复现，核心是 local-to-global 两阶段结构：
  1. 局部阶段（local）：GATConv 做边级等变局部注意力，结合多项式交互
     x = (1-beta)*LN(h·x) + beta*x，h = relu(h_lin(x)) 是独立自项。
  2. 全局阶段（global）：GlobalAttn 用 kernel-trick 线性全局注意力捕捉长程依赖。
训练时先 local_epochs 训练局部阶段（self._global=False，pred_local 输出），
再切 self._global=True 用全局注意力继续训练（pred_global 输出）。

在"官方 + 关系感知分支"的双分支实验中，本文件即官方保底分支：
它被 SafeRedesignedPolynormer / SafeRedesignedPolynormerSlim 直接复用。
"""
import torch
...
class GlobalAttn(torch.nn.Module):
    """官方 Polynormer 的全局注意力（linear global attention, kernel trick）。

    基于核技巧的线性注意力：k/v 经 sigmoid 映射后先聚合 kv，再用 q 做查询，
    复杂度 O(N) 而非 O(N^2)。逐层多项式混合 beta 控制每层全局更新强度。
    仅当模型的 self._global=True 时启用。
    """
...
class Polynormer(torch.nn.Module):
    """官方 Polynormer 主模型（我们的保底分支与复现基线）。

    局部层（local_layers 层）：
      pre_ln -> h=relu(h_lin(x)) -> GAT 消息 + 残差线性 -> relu/dropout
      -> x = (1-beta)*LN(h*x) + beta*x        # 多项式交互（每层标量 beta）
    全局层：self._global=True 时接 GlobalAttn + pred_global；否则 pred_local。
    """
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3, global_layers=2,
        ...
        self._global = False   # 阶段开关：False=局部阶段, True=全局阶段
